...
The type carries no precedence or associativity information, as these
are determined by the syntax tree. Recursive variants use \texttt{Box<Regex>}
because a direct definition \texttt{Seq(Regex, Regex)} would have infinite
size and \texttt{Box} stores these recursive values separately, providing finite
size~\citep{rust-book-box,rust-reference-recursive-types}. This differs from
Haskell's heap-allocated data types. Rust requires explicit indirection since
the enum sizes must be known at compile time.

Constructors encapsulate the boxing and are purely structural: they perform
no simplification, and \texttt{Regex::seq(Regex::Eps, r)} yields a \texttt{Seq}
node, not $r$. They take arguments by value, moving subexpressions into boxes.
