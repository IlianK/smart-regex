//! Level 3 rendering for standard derivative-based parser 
...
pub fn render(
    ...
    let (result, trace) = match config.parser_type {
        ParserType::DerivStdRec => parse_recursive_traced(input, r),
    ...
    let parser_label = match config.parser_type {
        ParserType::DerivStdRec  => "POSIX Standard (recursive parser)",
        ParserType::DerivStdLoop => "POSIX Standard (loop parser)",
        _                     => unreachable!(),
    };
