...
// RECURSIVE PARSER - TRACED VARIANT
...
mod tests {
    use super::*;
    use super::super::parse::{parse_deriv_std_rec, parse_deriv_std_loop};

    // parse_loop_traced
    #[test]
    fn loop_traced_matches_untraced_on_success() {
        let r = Regex::star(Regex::lit('a'));
        let plain = parse_deriv_std_loop("aaa", &r);
    ...
    fn loop_traced_matches_untraced_on_failure() {
        let r = Regex::star(Regex::lit('a'));
        let plain = parse_deriv_std_loop("aab", &r);
    ...
    fn recursive_traced_matches_untraced_on_success() {
        let r = Regex::star(Regex::lit('a'));
        let plain = parse_deriv_std_rec("aaa", &r);
    ...
    fn recursive_traced_matches_untraced_on_failure() {
        let r = Regex::star(Regex::lit('a'));
        let plain = parse_deriv_std_rec("aab", &r);
